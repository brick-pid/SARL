...
async def run_main_loop(
    ...
    *,
    ...
    while True:
        ...
        if _should_stop_on_repeat(recent_actions, current_action, max_repeat):
...
async def run_subagent_loop(
    ...
    *,
    ...
    for index, action in enumerate(action_list):
        obs_str += f"{action} -> {obs_list[index]}\n"
    obs = obs_str
...
def _should_stop_on_repeat(recent_actions: list[str], current_action: str, max_repeat: int) -> bool:
    return len(recent_actions) >= max_repeat - 1 and all(
        action == current_action for action in recent_actions[-(max_repeat - 1):]
    )
